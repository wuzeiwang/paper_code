...
if __name__ == '__main__':
    ...
    for i in axisx:
        # 神经网络
        now_bp_mse = BPRegression(X_train, y_train, X_test, y_test, i * model_max_epochs, hidden_unit_number,
                                  hidden_layer_num, regularization_learn_rate)
        history_bp_mse_lost.append(now_bp_mse - last_bp_mse)
        history_bp_mse.append(now_bp_mse)
        last_bp_mse = now_bp_mse
        # 神经网络Boosting
        ...
        now_bpboosting_mse_lost = tf_sklearn_network.boost_predict(model_max_epochs, network_number, X_test, y_test, i,
                                                                   hidden_unit_number, hidden_layer_num,
                                                                   regularization_learn_rate,
                                                                   train_model)
        history_bpboosting_mse_lost.append(now_bpboosting_mse_lost - last_bpboosting_mse_lost)
        history_bpboosting_mse.append(now_bpboosting_mse_lost)
        last_bpboosting_mse_lost = now_bpboosting_mse_lost
    ...
    plt.plot(axisx_bp, history_bp_mse, c="blue", label="netWork", linestyle='-', marker='v')
    plt.plot(axisx_bp, history_bpboosting_mse, c="red", label="NNBoost", linestyle='--', marker='o')

    # plt.plot(layer_axisx, history_bp_mse, c="blue", label="netWork", linestyle='-', marker='v')
    # plt.plot(layer_axisx, history_bpboosting_mse, c="red", label="netWorkBoost", linestyle='--', marker='o')
    # plt.plot([0, max(axisx)], [min_mse, min_mse])
    plt.legend()
    plt.xlabel('network_epochs_num')
    plt.ylabel('MSE')
